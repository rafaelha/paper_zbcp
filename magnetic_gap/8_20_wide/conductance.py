...
def plotG(en, N, Ree, Reh):
    ...
    N = np.block([N,N[cond]])[ind]
    Ree = np.block([Ree,Ree[cond]])[ind]
    Reh = np.block([Reh,Reh[cond]])[ind]
...
if __name__ == '__main__':
    """
    rg = np.linspace(0,130e-6,41)
    #rg = np.sort(np.block([rg, -rg]))
    loop(rg)

    """
    r = int(sys.argv[1])
    bz = np.linspace(0,Q*1.1,250)
    kz=bz[r]
    seed = -1
    start_time = time.time()
    rg = np.block([np.linspace(-700,-100,150)[:-1], np.linspace(0,20,55),
                   np.linspace(20,100,45)[1:],
                   np.linspace(100,700,150)[1:]])*1e-6
    plt.close('all')
    start_time = time.time()
    sys, lead0, lead1 = build_sys()
    en, N, Ree, Reh, G = sim(sys, range=rg, plot=False)
