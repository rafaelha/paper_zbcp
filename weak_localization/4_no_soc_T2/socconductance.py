...
W = 0.045 # disorder strength
Lx = 15
Ly = 300
Lz = 15

wvals = np.random.rand(Lx,Ly)
...
def disorder_t_inv(i,j):
        return (wvals[i,j] - 1/2) * W * kr(sigz, t0, s0)
    ...
    for i in np.arange(Lx):
        for j in np.arange(Ly):
            for k in np.arange(Lz):
                sys[(lat(i, j, k) )] = onsite() + disorder_t_inv(i,j)
    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    for i in np.arange(Lx * (1-xdir) + xdir):
        for j in np.arange(Ly * (1-ydir) + ydir):
            for k in np.arange(Lz * (1-zdir) + zdir):
...
if __name__ == '__main__':
    seed = int(sys.argv[1])
    np.random.seed(seed)
    rg = np.sort(np.block([np.linspace(0,120e-6,30)]))
    loop(rg)
    """
    start_time = time.time()
    sys, lead0, lead1 = build_sys()
