...
def DOS(sys, k=100, range=(-1.5*50e-6,1.5*50e-6), bins=1000, fignum=2): # plot the lowest eigenvalues in a hist-plot
    ...
    return ev
def build_sys():
    lat = kw.lattice.general([(ax,0,0), (0,ay,0), (0,0,az)], norbs=4)
    sys = kw.Builder()
...
if __name__ == '__main__':
    seed = int(sys.argv[1])
    np.random.seed(seed)
    rg = np.sort(np.block([np.linspace(-200e-6,-100e-6,5)[:-1], np.linspace(0,100e-6,24), np.linspace(100e-6,200e-6,5)[1:]]))
    loop(rg)
    """
    start_time = time.time()
    sys, lead0, lead1 = build_sys()
